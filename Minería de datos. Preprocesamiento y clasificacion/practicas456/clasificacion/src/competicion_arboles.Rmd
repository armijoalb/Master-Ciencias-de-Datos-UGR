...
Probamos a hacer imputación de valores para ver si mejoran los resultados.
```{r}
# Hacemos validación.
...
Volvemos a hacer validación cruzada.
```{r}
posicionClase <- length(names(new_train))
n = 1:10
cv = sapply(n,trainDataCVLMT,data=new_train,posicionClase=posicionClase)
...
Probamos a añadir tomek links que antes nos ha dado mejores resultados que smote y otros algoritmos de oversampling.
```{r}
aux = new_train
n = ncol(new_train)
output = new_train$C
...
Probamos a transformar los datos a otro espacio y comprobamos los resultados.
```{r}
library(kernlab)
library(GGally)
...
Probamos a hacer validación cruzada con los datos nuevos.
```{r}
...
No funciona demasiado bien por lo que probaremos a utilizar pca solamente.
```{r}
new_train = datos.imputados
...
Probamos a predecir los datos y ver que tal funciona el algoritmo.
```{r}
test.resultados = predict(best.model$modelo, newdata=datos.test, type="class")
...
Añadimos preprocesamiento con IPF y comprobamos si mejora el modelo.
```{r}
posicionClase <- length(names(new_train))
variableClase <- names(new_train)[posicionClase]
...
Probamos a utilizar oversampling y comprobamos si mejoran los resultados.
```{r}
aux = new_train
...
No ha empeorado mucho pero no ha conseguido mejorar lo que había hasta ahora, probaremos a hacer undersampling a los datos y evaluaremos los resultados.
```{r}
new_train = aux
n = ncol(new_train)
output = new_train$C
...
Al igual que para otros algoritmos, Tomek Links no ofrece buenos resultados, posiblemente debido a la pérdida de información producida por el algoritmo. Probaremos también con añadir datos con BLSMOTE después de utilizar TomekLinks, dado que antes ha funcionado bien, después se volverá a pasar IPF para eliminar ruido generado con BLSMOTE.
...
Realmente PCA no nos ofrece ningún beneficio en ninguna de sus variantes, ya que ni con transformaciones lineales ni con KPCA. Por lo que buscaremos otra forma de eliminar variables de nuestro dataset y comprobar si mejoran los resultados obtenidos con los modelos. Para ello utilizaremos boruta para intentar reducir el número de variables.
...
Según boruta todos los atributos son importantes, por lo cual no podemos eliminar ninguna variable con este método. Lo que si podemos hacer es probar a eliminar variables altamente correladas, y con eso volver a ejecutar lo que habíamos estado haciendo hasta el momento, y probar si mejora lo que hemos conseguido hasta ahora.
```{r}
new_train = datos.imputados
...
realizamos ipf, después blsmote, hacemos un modelo.
```{r}
posicionClase <- length(names(new_train))
variableClase <- names(new_train)[posicionClase]
